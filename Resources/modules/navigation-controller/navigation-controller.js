...
exports.NavigationController = function(defaults) {
	if(defaults){
		for(key in defaults){
			windowDefaults[key] = defaults[key];
		}
	}
...
exports.NavigationController.prototype.buildWindow = function(inOptions) {
	var options = {};
	for(key in windowDefaults)
		options[key] = windowDefaults[key];
	if(inOptions)
	...
	if(this.windowStack.length === 1) {
		if(Ti.Platform.osname === 'android') {
			...
			var containerWindow = Ti.UI.createWindow({
				orientationModes: windowDefaults['orientationModes']
			});
